...
const SwitchTrack = styled.span`
  background-color: ${props => props.mode === 'dark' ? '#8796A5' : '#aab4be'};
  border-radius: 10px;
  width: 100%;
  height: 100%;
  display: block;
...
function CustomSwitch(props) {
    ...
    return (
        <SwitchRoot className={`${checked} ${disabled} ${focusVisible}`}>
            <SwitchTrack>
                <SwitchThumb className={`${checked} ${disabled} ${focusVisible}`} />
            </SwitchTrack>
